...
using Completed;  ///Gives access to player script                                                       
        ...
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            if (newSlot != null)
            {
                ...
                if (Inventory.GetComponent<EquipmentSystem>() != null)
                {
                    ...
                    /// This is where I need to add item requirements.
                    //if the slot is empty
                    else
                    {

                        playerInfoRef = GameObject.Find("Player").GetComponent<Player>();

                        int strengthReq = 0;
                        int dexterityReq = 0;
                        int intelligenceReq = 0;

                        ItemOnObject itemRef = transform.GetComponent<ItemOnObject>();
                        for (int i = 0; i < itemRef.item.itemAttributes.Count; i++)
                        {
                            if (itemRef.item.itemAttributes[i].attributeName == "StrengthReq")
                            {
                                strengthReq = itemRef.item.itemAttributes[i].attributeValue;
                            }
                            else if (itemRef.item.itemAttributes[i].attributeName == "DexterityReq")
                            {
                                dexterityReq = itemRef.item.itemAttributes[i].attributeValue;
                            }
                            else if (itemRef.item.itemAttributes[i].attributeName == "IntelligenceReq")
                            {
                                intelligenceReq = itemRef.item.itemAttributes[i].attributeValue;
                            }
                        }
                        ...
                        for (int i = 0; i < newSlot.parent.childCount; i++)
                        {
                            if (newSlot.Equals(newSlot.parent.GetChild(i)))
                            {
                                ...
                                if (itemTypeOfSlots[i] == transform.GetComponent<ItemOnObject>().item.itemType && strengthReq < playerInfoRef.playerStrength && dexterityReq < playerInfoRef.playerDexterity && intelligenceReq < playerInfoRef.playerIntelligence)
                                {
            ...
            else
            {
                ...
                inventory.OnUpdateItemList();
                dropItem.transform.SetParent(boardHolder);
